...
declare module "packages/kdocs/pcweb/index" {
    import SdkBase, { BASE } from "libs/SdkBase";
    import { ISdkLanguageVue } from "libs/PlugBase";
    import { InitOption } from "libs/PlugBase";
     ...
     * 文件类型
     */
    export type FileType = "file" | "folder" | "sharefile" | "delfile" | "linkfolder" | "sharefolder" | "delfolder" | "delgroup" | "group";
    /**
     * @source
     * 选中的文件信息 (我的文档、团队文档)
     */
     ...
     * 选中的文件信息 (最近)
     */
    export type SelectedFileInLatest = {
        app_type: string;
        b64fname: string;
     ...
     * 右键菜单type
     */
    export type IMenuList = "openFolderNewTab" | "specific_search" | "share" | "star" | "set_popular" | "copy" | "moveto" | "create_shortcuts" | "rename" | "history" | "save_as_ent_template" | "wps_edit" | "download" | "remove" | "folder_link" | "copy_ent_link" | "set_teamtop" | "file_permission" | "recover" | "destory" | "exit_stared" | "catalog" | "remove_file_from_record" | "remove_file_and_record" | "exit" | "cancel_link" | "sort" | "refresh" | "create_folder" | "create_file" | "create_template_library" | "create_from_ent_template" | "rename_team" | "remove_team";
    /**
     * @source
     *
     * 静态配置
     ...
     */
    export interface IKdocsPcSDK extends ModuleBase<{
        kso: KsoModule;
        uikit: UikitModule;
    }> {
        set(config: IConfig): void;
        /**
         * 左侧菜单过滤 原名Aside.menus(1.1.0)
         *
         */
        on(action: "Aside.menus.filter", callback: ISame<{
            key: "latest" | "tags" | "share" | "entspace" | "enterprise" | "trash";
            text: string;
            icon?: string;
        }[]>): any;
        /**
           * 团队列表渲染插槽 原名Aside.team.render(1.1.0)
           * @param action
           * @param callback
           *
           * teams 团队列表数据
           * 文件左键点击事件dispatch("ROOT", "ecis_team_yun_list_target_click", file),文件右键点击事件this.$parent.dispatch("ROOT", "ecis_team_yun_list_context_menu", {
                event,
                id: file.id,
              })
              文件节点需加上class="mouse-context-item"
           */
        on(action: "AsideTeam.root.render", callback: () => BASE.TYPE.VUE): any;
        /**
         * 渲染列表标签，返回一个Vue实例(1.1.0)
         * @param action
         * @param callback
         */
        on(action: "Document.listItemBottom.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         * 拦截打开文件事件 原名Docs.openFile(1.1.0)
         * @param action
         * @param callback
         *
         */
        on(action: "Document.openFile.intercept", callback: (file: {
            ftype: string;
            name: string;
            [k: string]: any;
        }) => boolean): any;
        /**
         * 替换工具栏 原名Document.file.folder(1.1.0)
         * @param action
         * @param callback
         * upload:上传
         * entTemplate：企业模板
         ...
         *
         */
        on(action: "DocslistTopMenu.root.filter", callback: ISame<{
            key: "upload" | "entTemplate" | "addfile" | "moreMenus";
            route: Route;
            text: string;
            [k: string]: any;
        }[]>): any;
        /**
         * 筛选工具栏里新建 name可以区分项目，有些需要param参数来区分,不可过滤文件夹(1.1.0)
         * @param action
         * @param callback
         * actionList：新建列表信息
         * route:路由信息
         *
         */
        on(action: "DocslistTopMenu.addfile.filter", callback: (props: {
            actionList: {
                name: "entTemplate" | "pom" | "pof" | "otl" | "ppt" | "form" | "doc" | "xls";
                icon: string;
                text: string;
                param: {
                    openDate: string;
        ...
        /**
         * 过滤右键菜单  原名Document.rmenu(1.1.2)
         * 在宿主返回数据的时候，menus会有数据为空的状态，例如text为空，插件自行判断是否渲染
         * @param action
         * @param callback
         *
         */
        on(action: "Document.moreMenu.filter", callback: (param: {
            routeName: string;
            groupsInfo?: any;
            selectedList: {
                id?: number;
                ftype: "file" | "folder" | "sharefile" | "delfile" | "linkfolder" | "sharefolder" | "delfolder" | "delgroup" | "group";
                fname?: string;
                fileid?: number;
                fsize?: number;
                groupid?: number;
            ...
            route: any;
            menus: {
                command?: "share" | "set_label" | "star" | "exit_stared" | "set_popular" | "copy" | "moveto" | "rename" | "history" | "download" | "save_as_ent_template" | "remove_file_from_record" | "remove_file_and_record" | "catalog" | "wps_edit" | "specific_search" | "remove" | "folder_link" | "exit" | "set_teamtop" | "cancle_teamtop" | "recover" | "destory" | "file_permission";
                text?: string;
                divided?: boolean;
            }[];
        }) => {
        ...
        /**
         * 过滤团队列表右键菜单 原名Aside.team.rmenu(1.1.2)
         * @param action
         * @param callback
         */
        on(action: "AsideTeam.moreMenu.filter", callback: ISame<{
            key: "top" | "commonUsed" | "addUser" | "teamSetting" | "teamRename" | "docRole" | "enterpriseTemplate" | "deleteTeam";
            [k: string]: any;
        }[]>): any;
        /**
         * 右键自定义菜单
         * 暂时保留原有addItem(1.1.2)
         * @alias Document.moreMenu.expand
         */
        on(action: "Document.rmenu.custom", callback: (selectedList: {
            id?: number;
            ftype: "file" | "folder" | "sharefile" | "delfile" | "linkfolder" | "sharefolder" | "delfolder" | "delgroup" | "group";
            fname?: string;
            fileid?: number;
         ...
         * 左侧新建菜单列表过滤(1.1.1)
         */
        on(action: "Aside.createPanel.filter", callback: ISame<{
            id: string;
            name: "form" | "folder" | "team" | "uploadBtn";
            text: string;
            title: string;
            icon?: string;
            [k: string]: any;
         ...
         * @since 1.1.2
         * @param action
         * @param callback props 传参：selected：SelectedFile[] | SelectedFileInLatest[] 选中的文件列表
         */
        on(action: "DocslistTopMenu.selected.expand", callback: (params: {
            currentRoute: string;
            selectedList: SelectedFile[] | SelectedFileInLatest[];
        }) => BASE.TYPE.VUE): any;
        /**
         * 翻译全文文案，替换文案(1.1.0)
         * @param action
         ...
         */
        /**
         * 插槽，用以替换Aside.menus.addItems的用法(1.1.2)
         */
        on(action: "Aside.menus.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         *跳转路由局部刷新，跳转方式一ecis开头 例如  ecis/a(1.1.2)
         * @param action
         * @param callback
         */
        on(action: "Document.ecisRouter.render", callback: () => void): void;
        /**
         * 翻译
         *
         */
         ...
         * @param callback
         */
        on(action: "response", callback: ISame<AxiosResponse>): any;
        /**
         * 接口拦截(1.1.6)
         * @param action
         * @param callback
         */
        on(action: "request", callback: ISame<AxiosRequestConfig>): any;
        /**
         * 文件列表上方-面包屑-左侧工具栏左侧增加插槽(1.1.3)
         * @param action type 当前路由页面
         * @param callback props 传参：pageType：string 当前路由页面
         */
        on(action: "DocslistTopMenu.leftTool.left.expand", callback: (data: {
            pageType: string;
        }) => BASE.TYPE.VUE): any;
        /**
         * 文件列表上方-面包屑-左侧工具栏右侧增加插槽(1.1.3)
         * @param action type 当前路由页面
         * @param callback props 传参：pageType：string 当前路由页面
         */
        on(action: "DocslistTopMenu.leftTool.right.expand", callback: (data: {
            pageType: string;
        }) => BASE.TYPE.VUE): any;
        /**
         * 点击退出登陆拦截(1.1.4)
         ...
         * @param callback
         */
        on(action: "Header.logoutButton.intercept", callback: () => Promise<boolean>): any;
        /**
         * 团队文档-更多菜单-历史版本弹出框-更多菜单过滤 1.1.5
         */
        on(action: "HistoryVersionModal.moreMenu.filter", callback: (props: {
            menus: {
                key: "history" | "saveVersionAs" | "addVersionTag";
            }[];
        ...
        /**
         * 应用中心菜单 可屏蔽 可修改(1.1.4.1)
         * @param action
         * @param callback
         */
        on(action: "Header.applicationCenter.filter", callback: ISame<{
            key: "meeting" | "form" | "todo" | "rili" | "wpsnote" | "mycontacts" | "classadmin" | "mibiao";
            desc?: string;
            text?: string;
        }[]>): any;
        /**
         * 搜索结果的右键自定义菜单(1.1.6)
         * @param action
         * @param callback props 传参：selectedList：SelectedFile[] 选中的文件列表
         */
        on(action: "Search.moreMenu.expand", callback: (selectedList: SelectedFile[]) => BASE.TYPE.VUE): any;
        /**
         * 文件列表的右键自定义菜单(1.1.6)
         * @param action
         * @param callback props 传参：selectedList 选中的文件列表
         */
        on(action: "Document.moreMenu.expand", callback: (props: {
            currentRoute: string;
            selectedList: SelectedFile[] | SelectedFileInLatest[] | SelectedFileInTag[];
        }) => BASE.TYPE.VUE): any;
        /**
         * 模板库按钮拦截(1.1.6)
         */
        on(action: "Aside.templateIcon.intercept", callback: () => Promise<boolean>): void;
        /**
         * 新建按钮拦截(1.1.6)
         * @param action
         * @param callback props 传参：type 菜单类型。备注：上传按钮、上传子菜单中的文件夹和文件按钮，均不做事件拦截
         */
        on(action: "Aside.createIcon.intercept", callback: (props: {
            type: "otl" | "ppt" | "pom" | "pof" | "form" | "dbt" | "team" | "doc" | "xls" | "folder";
        }) => Promise<boolean>): void;
        /**
         * 右键菜单拦截(1.1.6)
         * @param action
         * @param callback props 传参：type 点击类型， selectedFileIds 选中的文件id数组。备注：上传文件/上传文件夹菜单项不做事件拦截
         */
        on(action: "Document.moreMenu.intercept", callback: (props: {
            type: IMenuList;
            selectedFileIds: number[];
        }) => Promise<boolean>): void;
        /**
         * 列表顶部工具栏新建按钮菜单拦截(1.1.6)
         * @param action
         * @param callback type 菜单类型
         */
        on(action: "DocslistTopMenu.createIcon.intercept", callback: (type: "entTemplate" | "pom" | "pof" | "otl" | "ppt" | "form" | "dbt" | "doc" | "xls" | "folder" | "new_shortcuts") => Promise<boolean>): void;
        /**
         * 右上角用户卡片菜单过滤(1.1.6)
         * @param action
         * @param callback
         */
        on(action: "Header.userPanel.filter", callback: ISame<{
            key: "work" | "logout";
        }[]>): void;
    }
    /**
     * @interfaceContent
     */
    export interface IKdocsAttach extends InitOption<ISdkLanguageVue> {
        /**
         * 拦截器对象
         */
        interceptors: {
     ...
     * @notExported
     */
    export namespace KDOCS_PCWEB {
        type SDK = IKdocsPcSDK;
        type Attach = IKdocsAttach;
    }
    /**
     *
     */
    export class KdocsPcSDK extends SdkBase {
        on(name: string, callback: (data?: any) => any): void;
    }
}
