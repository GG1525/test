...
declare module "packages/wpsplus/work_h5/index" {
    ...
    export interface IConfig {
         ...
         * 路由信息配置
         */
        Router: Array<RouterItem>;
    }
    export type ISame<T> = (p: T) => T;
    export interface IWorkPlugSDK {
        set(data: IConfig): void;
        /**
         * 浏览器页签
         * @param action
         * @param callback
         */
        on(action: "Header.browserTab.reset", callback: (title: string) => {
            title: string;
            icon: string;
        }): void;
        /**
         * 企业信息页底部拓展（新增）
         * @param action
         * @param callback （返回Function() : React组件）
         */
        on(action: "CompanyInfo.menu.expand", callback: () => () => BASE.TYPE.REACT): void;
        /**
         * 企业成员页列表项点击拦截（新增）
         * @param action
         * @param callback
         */
        on(action: "CompanyMember.menu.intercept", callback: (arg: any) => boolean): void;
        /**
         * i18n国际化全局文案
         * @param action
         * @param callback
         ...
         */
        mountToBody(component: BASE.TYPE.REACT): void;
    }
}
