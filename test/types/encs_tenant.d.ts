...
declare module "packages/encs/tenant/index" {
     ...
     * @source
     */
    export interface IDataDetailItem {
        imgSrc: string;
        dataDetails: {
            text: string;
            value: string | number;
     ...
     * @declaration
     */
    export interface ITenantPlugSdk extends ModuleBase {
        /**
         * 设置静态配置
         * @param data
         */
        ...
        /**
         * 左侧菜单路由设置，可对菜单进行新增、删除、修改图标和页面组件 (1.1.4)
         * 注：新增一级菜单需通过Router.config.reset新增路由菜单信息，通过Router.permission.reset同步新增权限数据结合使用；新增二级子菜单无权限概念，只要父级菜单有权限，子菜单默认有权限，只需要通过Router.config.reset新增路由菜单信息即可新增二级子菜单；
         * 项目加载默认菜单是数据统计，不能删除该菜单
         */
        on(action: "Router.config.reset", callback: ISame<IRouterData[]>): any;
        /**
         * 左侧菜单权限设置，可对菜单权限进行新增、删除 (1.1.4)，
         * 注：项目加载默认菜单是数据统计，不能删除该菜单
         */
        on(action: "Router.permission.reset", callback: (data: {
            roleId: string;
        ...
        /**
         * 各类图标色修改，数据概况过滤，需要把数据概况信息传给插件1.1.5
         * @param event
         * @param callback
         */
        on(event: "DataStatistic.dataOverviewList.filter", callback: (props: {
            dataDetailItem: IDataDetailItem[];
        }) => IDataDetailItem[]): void;
        /**
         * 响应拦截器 非加密环境取url 加密环境取$$url 1.1.6
         * @param action
         * @param callback
         */
     ...
     * @interfaceContent
     */
    export type ITenantAttach = InitOption<ISdkLanguageReact>;
    /**
     * @notExported
     */
    export namespace ENCS_TENANT {
        type SDK = ITenantPlugSdk;
        type Attach = ITenantAttach;
    }
    export class TenantPlugSdk extends SdkBase implements ITenantPlugSdk {
    }
}
