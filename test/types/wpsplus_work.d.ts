...
declare module "packages/wpsplus/work/index" {
    ...
    import type { MemberStatus } from "extensions/company/index";
    /**
     * 路由表配置项
     * @source
     */
     ...
     * @source
     */
    export type deptButtonsKey = "inviteJoin" | "addMember" | "adjustDept" | "sortMember" | "resetPwd" | "enableAndDisable" | "delete";
    /**
     * 组织架构选中项
     * @source
     */
    export type deptSelected = {
        account: string;
        avatar: string;
        city: string;
        comp_id: number;
     ...
     * @source
     */
    export type tableOperationItems = {
        type: string;
        command: string;
        text: string;
        disabled: boolean;
     ...
     * @source
     */
    export type docSettingItem = {
        title: string;
        decs: string;
        /**
         * saveSecurityDoc 默认存安全文档格式; foreSaveSecurity 强制开启安全文档保护; forbidSaveNormal 安全文档禁止解密
         */
        type: "saveSecurityDoc" | "foreSaveSecurity" | "forbidSaveNormal";
    };
    /**
     * 水印内容
     * @source
     */
    export type WatermarkContents = {
        label: string;
        type: number;
        control: number;
        shouldShow: boolean;
     ...
     */
    export interface IWorkPlugSDK extends ModuleBase<{
        kso: KsoModule;
        uikit: UikitModule;
    }> {
        set(data: IConfig): void;
        /**
         * 组织架构table列定制（新增、删除）1.1.0
         * @param action
         * @param callback
         */
        on(action: "DepartmentPanel.tableRow.filter", callback: ISame<colConf[]>): void;
        /**
         * 企业模板-模板图片替换   1.1.4
         * @param action
         * @param callback
         * imgSrc 图片地址
         */
        on(action: "CompTemplate.Template.reset", callback: (imgSrc: string) => string): void;
        /**
         * 请求拦截器 1.1.0
         * @param action
         * @param callback
        ...
        on(action: "response", callback: (config: AxiosResponse) => AxiosResponse): void;
        /**
         * 组织架构-列表顶部成员操作栏按钮过滤 1.1.1
         * @param action
         * @param callback
         */
        on(action: "DepartmentPanel.listTopMenu.filter", callback: (params: {
            key: deptButtonsKey;
            row: deptSelected[];
        }[]) => {
            key: deptButtonsKey;
        ...
        /**
         * 组织架构table操作项过滤 1.1.1
         * @param action
         * @param callback
         */
        on(action: "DepartmentPanel.tableOperation.filter", callback: (params: {
            items: tableOperationItems[];
            row: deptSelected;
        }) => tableOperationItems[]): void;
        /**
         * 左侧菜单文案、图标替换 1.1.1
         * @param action
         * @param callback
         */
        on(action: "Aside.menu.icon", callback: ISame<menu[]>): void;
        /**
         * 新增菜单路由 1.1.1
         * @param action
         * @param callback
         */
        on(action: "Router.config.reset", callback: ISame<routerList[]>): void;
        /**
         * 安全控制-安全策略设置-安全文档设置项(默认存安全文档格式、强制开启安全文档保护、安全文档禁止解密)屏蔽、文案和描述修改 1.1.2
         * @param action
         * @param callback
         */
        on(action: "SecurityPolicySetting.docSetting.filter", callback: ISame<docSettingItem[]>): void;
        /**
         * 文档管理-团队设置项屏蔽 1.1.2
         * @param action
         * @param callback departmentGroup 创建部门时自动创建关联部门的团队; groupApproval 创建团队需要管理员审批; secureGroup 新建团队默认开启文档加密保护; selectDepartmentMembers 添加成员到团队时，允许全选部门成员
         */
        on(action: "GeneralSetting.group.filter", callback: ISame<{
            key: "departmentGroup" | "groupApproval" | "secureGroup" | "selectDepartmentMembers";
        }[]>): void;
        /**
         * 全文翻译 1.1.3
         * @param action
        ...
        /**
         * 水印弹框水印显示内容过滤修改 1.1.3（1.1.4版本已废弃）
         * @param action
         * @param callback
         */
        on(action: "WatermarkModal.watermarkContents.filter", callback: (param: WatermarkContents[]) => WatermarkContents[]): void;
        /**
         * 空间自定义设置页面重绘 1.1.3
         * @param action
         * @param callback
         */
        on(action: "SpaceSetting.container.render", callback: (param: {
            type: "corpspecial" | "corpdep" | "custom";
            page: string;
            title: string;
            data: {
        ...
        /**
         * 成员激活状态统计插槽 和 导出excel文件到本地插槽 1.1.5
         * @param action
         * @param callback
         */
        on(action: "DepartmentPanel.MembersRecursive.expand", callback: (param: MemberFilterInfo) => BASE.TYPE.VUE): any;
        /**
         * 企业模板页面空页面时替换图片地址 1.1.5
         * @param action
         * @param callback
         */
        on(action: "CompanyTemplate.emptyImg.reset", callback: (params: string) => string): any;
        /**
         * 右上角用户卡片菜单过滤(1.1.6)
         * @param action
         * @param callback
     ...
     * @interfaceContent
     */
    export interface IWorkAttach extends InitOption<ISdkLanguageVue> {
        interceptors: {
     ...
     * @notExported
     */
    export namespace WPSPLUS_WORK {
        type SDK = IWorkPlugSDK;
        type Attach = IWorkAttach;
    }
}
