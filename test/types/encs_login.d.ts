...
declare module "packages/encs/login/index" {
    import SdkBase from "libs/SdkBase";
     ...
     * @declaration
     */
    export interface ILoginPlugSDK extends ModuleBase {
         ...
         * @param callback
         */
        on(action: "Header.logo.expand", callback: () => ReactElement): any;
        /**
         * 修改密码框替换
         * @param action
         * @param callback
         * isExpired 是否过期
         */
        on(action: "ModifyPasswordPage.render", callback: (data: {
            isExpired: boolean;
            config: LonginConfig;
        }) => ReactElement): void;
        /**
         ...
         * @param action
         * @param callback
         * password1: 新密码 password2：确认密码 setErrormsg：设置错误提示信息函数 config
         */
        on(action: "ModifyPasswordPage.ModifyPasswordButton.intercept", callback: (data: {
            password1: string;
            password2: string;
         ...
         * @param callback
         */
        on(action: "TenantLoginPage.loginButton.intercept", callback: (data: {
            code: string;
            setErrormsg: (data: string) => void;
            config: LonginConfig;
        }) => Promise<boolean>): void;
         ...
         * @param callback
         */
        on(action: "SSOLoginPage.loginButton.intercept", callback: (data: {
            account: string;
            password: string;
            needCaptcha: boolean;
            captcha: string;
         ...
         * @param callback
         */
        on(action: "LoginPage.forgotPasswordButton.intercept", callback: (data: {
            config: LonginConfig;
        }) => Promise<boolean>): void;
        /**
         * 登录框forminput下新增插槽
         ...
         * @param callback
         */
        on(action: "LoginPage.input.expand", callback: (data: {
            config: LonginConfig;
        }) => ReactElement): void;
        /**
         * 登录框form底部新增插槽
         ...
         * @param callback
         */
        on(action: "LoginPage.bottom.expand", callback: (data: {
            config: LonginConfig;
        }) => ReactElement): void;
        /**
         * 登录整个页面替换
         ...
         * @param callback
         */
        on(action: "LoginPage.render", callback: (data: {
            config: LonginConfig;
        }) => ReactElement): void;
        /**
         * 点击登录按钮拦截
         ...
         * @param callback
         */
        on(action: "LoginPage.loginButton.intercept", callback: (data: {
            account: string;
            password: string;
            needCaptcha: boolean;
            captcha: string;
         ...
         * @param callback
         */
        on(action: "LoginPage.verifySuccess.intercept", callback: (data: {
            result: ILoginCallback;
            config: LonginConfig;
            loginContext: LonginContext;
            succHandle: (result: ILoginCallback) => void;
        }) => Promise<boolean>): void;
        /**
         * 点击登录-verify-失败
         * @param action
         * @param callback
         */
        on(action: "LoginPage.verifyError.intercept", callback: (data: {
            result: ILoginFail;
            config: LonginConfig;
            loginContext: LonginContext;
            errHandle: {
         ...
         * @param callback
         */
        on(action: "LoginPage.loginAjaxSuccess.intercept", callback: (data: {
            ssid: string;
            config: LonginConfig;
            loginContext: LonginContext;
        }) => Promise<boolean>): void;
         ...
         * @param callback
         */
        on(action: "LoginPage.beforeLoginAjax.intercept", callback: (data: {
            ssid: string;
            config: LonginConfig;
            loginContext: LonginContext;
        }) => Promise<boolean>): void;
         ...
         * @param callback
         */
        on(action: "LoginPage.loginAjaxError.intercept", callback: (data: {
            ssid: string;
            config: LonginConfig;
            loginContext: LonginContext;
        }) => Promise<boolean>): void;
        ...
        /**
         * 请求拦截器    加密环境下使用(1.0.10)
         * @param action
         * @param callback
         */
        on(action: "instance.request", callback: (config: AxiosRequestConfig) => AxiosRequestConfig): void;
        /**
         * 插入元素，根节点下新增插槽使用
         * @param child
         * @param callback
         */
        insertDom(child: Node): () => void;
    }
    /**
     * @interfaceContent
     */
    export interface ILoginAttach extends InitOption<ISdkLanguageReact> {
        interceptors: {
         ...
         * 获取登陆路径
         */
        getLoginUrl: (query?: string) => string;
        /**
         * 跳转到指定的url
         */
        jumpToUrl: (url: string, paramStr: string, noSearch?: boolean, checkClient?: boolean) => void;
        /**
         * 是否在wps客户端内
         */
        isWps: boolean;
     ...
     * @notExported
     */
    export namespace ENCS_LOGIN {
        type SDK = ILoginPlugSDK;
        type Attach = ILoginAttach;
    }
    export class LoginPlugSDK extends SdkBase implements ILoginPlugSDK {
        insertDom(child: Node): () => void;
    }
}
