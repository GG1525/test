...
declare module "packages/kdocs/dotview/index" {
    ...
    export type MoreMenu = {
         ...
         * share 二级菜单分享
         */
        key: "shareTree" | "share" | "saveas" | "move" | "export" | "catalog" | string;
        /**
         * 菜单文案
         */
        text: "分享" | "分享" | "另存为" | "移动" | "下载" | "打开文件位置" | string;
        [k: string]: any;
    };
    /**
     * 文件信息
    ...
    export interface IKdocsPcSDK extends ModuleBase<{
        ...
        /**
         * 文件菜单，根据key过滤菜单 1.1.3
         * @param action
         * @param callback
         */
        on(action: "Header.moreMenu.filter", callback: ISame<MoreMenu[]>): any;
        /**
         * 菜单拓展 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： file：FileInfo（文件信息）
         */
        on(action: "Header.moreMenu.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         * 头部左侧插槽，返回一个Vue实例 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： file：FileInfo（文件信息）
         */
        on(action: "Header.leftContainer.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         * 头部中级插槽，返回一个Vue实例 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： file：FileInfo（文件信息）
         */
        on(action: "Header.centerContainer.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         * 头部右侧插槽，返回一个Vue实例 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： file：FileInfo（文件信息）
         */
        on(action: "Header.rightContainer.expand", callback: () => BASE.TYPE.VUE): any;
        /**
         * 内容区域的替换 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： file：FileInfo（文件信息）
         */
        on(action: "Document.fileDetail.render", callback: (file: {
            fileInfo: FileInfo;
        }) => BASE.TYPE.VUE): any;
        /**
         * 拦截下载按钮 1.1.3
         ...
         * @param callback
         */
        on(action: "Document.downloadButton.intercept", callback: (file: {
            fileInfo: FileInfo;
            userInfo: UserInfo;
        }) => Promise<boolean>): any;
        /**
         ...
         * @param callback
         */
        on(action: "Document.downloadButton.visible", callback: (file: {
            fileInfo: FileInfo;
            userInfo: UserInfo;
        }) => Promise<boolean>): any;
        /**
         * 拦截左侧菜单按钮 1.1.3
         * @param action
         * @param callback command 左侧菜单按钮类型
         */
        on(action: "Header.moreMenu.intercept", callback: (file: {
            fileInfo: FileInfo;
            command: string;
        }) => Promise<boolean>): any;
        /**
         * 拦截右侧分享按钮点击事件 1.1.3
         * @param action
         * @param callback isSecret：是否是私密文件
         */
        on(action: "Header.shareButton.intercept", callback: (file: {
            fileInfo: FileInfo;
            isSecret: boolean;
        }) => Promise<boolean>): any;
        /**
         ...
         * @param callback
         */
        on(action: "Document.file.render", callback: (file: {
            fileInfo: FileInfo;
        }) => BASE.TYPE.VUE): any;
        /**
         * 内容区域的图标替换 1.1.3
         * @param action
         * @param callback
         * 自定义组件参数： type(icon类型) 、 ecisFileName（文件的fname）
         */
        on(action: "Document.fileIcon.render", callback: () => BASE.TYPE.VUE): any;
        /**
         * 浏览器页签图标定制 1.1.3
         * @param action
         * @param callback
         * iconType 图标类型  ecisFileName 图标名称
         */
        on(action: "Header.browserTab.icon.reset", callback: (data: {
            iconType: string;
            ecisFileName: string;
        }) => string): any;
        /**
         * 页面初始化拦截 1.1.3
         * @param action
         * @param callback 该api的fileInfo是异步获取的值，若在插件中使用需通过监听机制获取
         */
        on(action: "Document.file.intercept", callback: (file: {
            fileInfo: FileInfo;
        }) => Promise<boolean>): any;
    }
    /**
     * IDotviewAttach为插件方法的第二个参数对象
     * @interfaceContent
     */
    export interface IDotviewAttach extends InitOption<ISdkLanguageVue> {
        /**
         * 拦截器对象
         * @notExported
         */
     ...
     * @notExported
     */
    export namespace ENCS_DOTVIEW {
        type SDK = IKdocsPcSDK;
        type Attach = IDotviewAttach;
    }
}
