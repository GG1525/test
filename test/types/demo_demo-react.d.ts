...
declare module "packages/demo/demo-react/index" {
    import { BASE } from "libs/SdkBase";
    export interface IReactPlugSdk {
        on(action: "button.render", callback: ({ sourse }: {
            sourse: number;
        }) => BASE.TYPE.REACT): any;
        on(action: "Router", callback: (path: string) => any): any;
