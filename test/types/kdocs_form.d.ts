...
declare module "packages/kdocs/form/index" {
    ...
    export interface IFormInfo {
         ...
         * 表单类型
         */
        formType: "newform" | "newclockin" | "exam";
    }
    /**
     * @source
     * 应用中心信息
    ...
    export interface IConfig {
        ...
        Header?: {
            /**
             * 浏览器页签，title需要用  translate  方法修改 （1.1.4）(pc、h5)
             */
            browserTab?: {
                icon: string;
            };
            /**
             * 左上角logo与公司名（1.1.4） (h5/pc)
             */
            logo?: {
                /**
                 * 左上角logo图标
     ...
     */
    export interface IKdocsFormPlugSDK extends ModuleBase<{
        kso: KsoModule;
        uikit: UikitModule;
    }> {
         ...
         * @param callback
         */
        on(action: "InviteModal.right.expand", callback: (formInfo: IFormInfo) => BASE.TYPE.REACT): any;
        /**
         * 用户面板下拉框过滤 1.1.4 pc
         * @param action
         * @param callback commonInfoManager 常用信息管理；logOut 退出登陆
         */
        on(action: "Header.userPanel.filter", callback: ISame<{
            key: "commonInfoManager" | "logOut";
        }[]>): any;
        /**
         * 二维码中心图标替换 1.1.4 pc
         * @param action
         ...
         * 返回的string为照片base64
         */
        on(action: "QRcode.icon.reset", callback: (iconUrl: string) => string): any;
        /**
         * 海报模板弹窗屏蔽 1.1.4 pc
         * @param action
         * @param callback
         */
        on(action: "QRcode.templateModal.intercept", callback: (url: string) => Promise<boolean>): any;
        /**
         * 包含企业logo的二维码缩略图替换 1.1.4 pc
         * @param action
         * @param callback
         */
        on(action: "QRcode.thumbnail.reset", callback: (iconUrl: string) => string): void;
        /**
         * 过滤修改应用中心 1.1.4 pc
         * @param action
         * @param callback
         */
        on(action: "Header.applicationCenter.filter", callback: (props: ApplicationCenter[]) => ApplicationCenter[]): void;
        /**
         * 翻译 1.1.4 pc/h5
         */
        ...
        /**
         * 分享tab邀请分享按钮拓展 1.1.5 pc
         */
        on(action: "InviteTab.linkButton.expand", callback: (formInfo: IFormInfo) => BASE.TYPE.REACT): any;
        /**
         * 接口拦截(1.1.5)
         * @param action
         * @param callback
        ...
        /**
         * 限制填写人范围设置开关 1.1.5 pc/h5
         * 当填写范围为开启时，再次关闭开启，选择人员会重置，在h5页面填写范围为单独页面，所以在中插件根据条件自行判断
         * @param action
         * @param callback
         */
        on(action: "LimitScope.button.default", callback: (param: {
            setLimitScope: (boolean: any) => void;
        }, specifiedContactsLength: number) => void): any;
        /**
         * 填写范围重绘 1.1.5 pc/h5
         * @param action
         * @param callback
         */
        on(action: "CreateForm.writePermission.render", callback: (param: {
            contactItems: {
                avatar?: string;
                company_id?: string;
            ...
            setContactShow: (boolean: any) => void;
        }) => BASE.TYPE.REACT): any;
    }
    /**
     * IFormAttach为插件方法的第二个参数对象
     * @interfaceContent
     */
    export interface IFormAttach extends InitOption<ISdkLanguageReact> {
        /**
         * 拦截器对象
         */
        interceptors: {
    ...
    export namespace ENCS_FORM {
        type SDK = IKdocsFormPlugSDK;
        type Attach = IFormAttach;
    }
}
