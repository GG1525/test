...
declare module "packages/kdocs/h5/index" {
    import { BASE } from "libs/SdkBase";
     ...
     * 新增面板团队信息
     */
    export interface INewPanelGroupInfo {
        id?: number;
        memberCount?: number;
        recentMembers: {
            avatar: string;
            nickName: string;
            role?: "creator" | "admin" | "read_member" | "member";
            id: number;
            status: string;
        }[];
        name?: string;
     ...
     * 文件分享默认参数
     */
    export interface IFileLinkDefaultParams {
        fileId: number;
        status: string;
        range: "anyone" | "company";
        permission: "read" | "comment" | "write" | "close";
        period?: string;
    }
    /**
     * @source
     * 文件夹分享默认参数
     */
    export interface IFolderLinkDefaultParams {
        folderId: number;
        range: "anyone" | "company";
    }
    /**
     ...
     * 更多面包信息
     */
    export interface IMorePanelInfo {
        creatorName: string;
        ctime?: number;
        currentPage: string;
        groupId: number;
     ...
     * 选择成员列表
     */
    export interface ISelectMemberList {
        avatar: string;
        companyId?: number;
        id: string;
        name?: string;
     ...
     * 选中成员列表
     */
    export interface IDeptCheckedList {
        avatar: null | string;
        companyId: number;
        id: string;
     ...
     * 文件夹信息
     */
    export interface INewPanelFolderInfo {
        corpId: number;
        groupId: number;
        id: number;
        type: string;
    ...
    export interface IKdocsH5Data {
        ...
        Footer?: {
            tabs?: {
                /**
                 * home 首页  file 文档  group 团队  me 我
                 */
                default?: "home" | "file" | "group" | "me";
            };
        };
        /**
         * @since 1.1.4.1
    ...
    type ISame<T> = (p: T) => T;
    /**
     * inviteByQRcode：二维码邀请
     * copyToInvite： 复制链接邀请好友加入
     * chooseFromContact：从联系人中选择
     */
    export interface InviteContactMenu {
        key: "inviteByQRcode" | "copyToInvite" | "chooseFromContact";
    }
    /**
     * @declaration
     * @ModuleBase [kso,uikit]
     */
    export interface IKdocsH5PlugSDK extends ModuleBase<{
        kso: KsoModule;
        uikit: UikitModule;
    }> {
        /**
         ...
         * shareInfo：当前分享信息
         * fileId：当前文件id，分享文件时
         * folderId：当前文件夹id，分享文件夹时
         */
        on(event: "SharePage.icon.expand", callback: (props: {
            shareInfo: IShareInfo;
            fileId?: string;
            folderId?: string;
        }) => ReactNode): void;
         ...
         * @param event
         * @param callback
         * key: home 首页 file 文件  group 团队 me 我
         * text:tab文本
         * icon:tabicon
         */
        on(event: "Footer.tab.filter", callback: ISame<{
            key: "home" | "file" | "group" | "me";
            title: string;
         ...
         * groupInfo:团队信息
         * ToolShowList：面板列表信息
         * type:word 文字  table 表格  ppt 演示 form 表单 team 团队 folder 文件夹  upload 上传
         */
        on(event: "NewPanel.root.filter", callback: (props: {
            ToolShowList: {
                id: number;
                imgSrc: string;
                text: string;
                type: "word" | "table" | "ppt" | "form" | "team" | "folder" | "upload" | "otl";
            }[];
            folderInfo?: INewPanelFolderInfo | undefined | any;
            groupInfo?: INewPanelGroupInfo | undefined | any;
        }) => {
            id: number;
            imgSrc: string;
            text: string;
         ...
         * goback:返回
         */
        on(event: "InviteContact.inviteButton.intercept", callback: (props: {
            groupId: string;
            chooseList: ISelectMemberList[];
            goback: () => void;
        }) => Promise<boolean>): void;
        /**
         * 过滤联系人操作项(1.1.3)
         ...
         * chooseFromContact：从联系人中选择
         */
        on(event: "InviteContact.menu.filter", callback: (props: InviteContactMenu[]) => InviteContactMenu[] | []): void;
        /**
         * 邀请成员过滤是否可选(1.1.3)
         * @param event
         * @param callback
         * id:选中人员id
         */
        on(event: "InviteContact.memberChecked.reset", callback: (props: {
            avatar: string;
            companyId: number;
            id: string;
            name: string;
         ...
         * exitGroup退出团队
         */
        on(event: "InviteContact.memberListBottom.expand", callback: (props: {
            userRole: "dissolveGroup" | "exitGroup";
        }) => BASE.TYPE.REACT): void;
        /**
         * 分享邀请成员拦截(1.1.3)
         * @param event
         * @param callback
         ...
         * goback:返回
         */
        on(event: "InviteContact.ShareInviteButton.intercept", callback: (props: {
            groupId: number;
            shareId: string;
            fileId: number;
            chooseList: ISelectMemberList[];
            deptCheckedList: IDeptCheckedList[];
            goback: () => void;
        }) => Promise<boolean>): void;
        /**
         * 翻译(1.1.3)
        ...
        /**
         * 文档页面顶部筛选器下方新增插槽 (1.1.3)
         * @param event
         * @param callback
         */
        on(action: "MyEntDocumentPage.filterBottom.expand", callback: (param: IGroupInfo) => BASE.TYPE.REACT): any;
        /**
         * 文件列表更多面板头部下方增加插槽 (1.1.3)
         *  @param event
         * @param callback
         */
        on(action: "MorePanel.headerBottom.expand", callback: (param: IMorePanelInfo) => BASE.TYPE.REACT): any;
        /**
         * 点击退出登陆拦截(1.1.4)
         * @param action
         * @param callback
         */
        on(action: "UserInfoPage.logoutButton.intercept", callback: () => Promise<boolean>): any;
        /**
         * 列表描述修改(1.1.5)
         * @param action
         * @param callback
         */
        on(action: "HomePage.fileListDescribe.reset", callback: (param: string) => string): any;
        /**
         * 文件分享默认值修改(1.1.6)
         * @param action
         * @param callback
         */
        on(action: "SharePage.fileLink.defaultValue", callback: (param: {
            fileInfo: {
                groupId: number;
                fileId: number;
            };
            defaultParams: IFileLinkDefaultParams;
        }) => IFileLinkDefaultParams): any;
        /**
         * 文件夹分享默认值修改(1.1.6)
         * @param action
         * @param callback
         */
        on(action: "SharePage.folderLink.defaultValue", callback: (param: {
            folderInfo: {
                groupId: number;
                folderId: number;
            };
            defaultParams: IFolderLinkDefaultParams;
        }) => IFolderLinkDefaultParams): any;
        /**
         * 分享权限范围过滤(1.1.6)
         * @param action
         * @param callback
         */
        on(action: "SharePage.permission.filter", callback: (param: {
            index: number;
            text: string[];
            text_en: string[];
        }) => {
         ...
         * @param callback
         */
        on(action: "InviteCollaborator.memberPermission.filter", callback: (props: string[]) => string[]): any;
    }
    /**
     * @interfaceContent
     */
    export interface IKdocsH5Attach extends InitOption<ISdkLanguageReact> {
        /**
         * 拦截器对象
         */
        interceptors: {
    ...
    export namespace KDOCS_PCWEB {
        type SDK = IKdocsH5PlugSDK;
        type Attach = IKdocsH5Attach;
    }
}
