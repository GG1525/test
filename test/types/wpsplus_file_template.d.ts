...
declare module "packages/wpsplus/file_template/index" {
    ...
    export interface IConfig {
         ...
         */
        Router: {
            [key: string]: Array<routerItem>;
        };
    }
    export interface IFileTemplPlugSDK {
        set(data: IConfig): void;
        /**
         * 替换企业模板为空的引导区域（新增）
         * @param action
         * @param callback (返回Vue组件)
         */
        on(action: "Root.emptyContent.render", callback: () => BASE.TYPE.VUE): void;
        /**
         * 顶部右侧区域自定义组件 （名称修改）
         * @param action
         * @param callback (返回Vue组件)
         */
        on(action: "Header.toppingPanel.expand", callback: () => BASE.TYPE.VUE): void;
        /**
         * 全局文案修改
         * @param action
         * @param callback
         */
        on(action: "translate", callback: (param: transParams) => string): void;
        /**
         * 请求拦截器
         * @param action
         * @param callback
