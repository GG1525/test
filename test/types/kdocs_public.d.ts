...
declare module "packages/kdocs/public/index" {
    ...
    export interface IConfigBase {
        Header?: Partial<{
            ...
            /**
             * 浏览器tab选项卡显示（1.1.4）
             * folderShare 前缀 folderSharePage
             * message 无
             * icon 图标
             * title 文案
             */
        ...
        /**
         * 新增路由(path必须要两级目录)例如  /ecis/super(1.1.2)
         */
        Router?: Partial<{
            [key: string]: {
                path: string;
                name?: string;
                component: BASE.TYPE.VUE;
            }[];
        }>;
        /**
         * 消息中心 1.1.4.1
     ...
     * 消息卡片按钮信息
     */
    export interface IMessagebuttonsInfo {
        /**
           * 按钮打开类型，如果打开类型填了，按钮就会按照以下方式打开，优先级最高
            cooperation: 对应params应返回id(文件id)，name(带后缀文件名)
            file: 对应params应返回id(文件id), sid(分享id)，name(带后缀文件名)
            shareFolder：对应params应返回id(团队id)，name(团队名)
            teamFolder：对应params应返回companyId(企业id)，teamId(团队id)，name(团队名)
            enterprise：对应params应返回id(企业id)，name(企业名)
            link：对应params应返回url(具体打开链接)
           */
        openType?: string;
        /**
         * 参数，具体看openType说明
         * openType存在时，creatorId（消息发起者ID）,creator(消息发起者名称)
         */
        ...
        url?: string;
        /**
         * 打开方式，可选值：blank（新窗口打开）、self（当前窗口），如果为空则blank新窗口打开
         */
        openTarget?: "blank" | "self";
        /**
         * http请求方法GET/PUT/POST/DELETE，如果为空并且url不为空的时候则以新窗口打开url
         */
        method?: string;
    }
    /**
    ...
    export interface IMessageInfo {
        ...
        avatar: string;
        /**
         * tab栏 cooperation 办公与协作，contact 新的联系人，schedule日常消息，application应用消息
         */
        category: string;
        /**
         * 消息类型
         * group_member_added 被添加入共享文件夹/个人团队，
         * file_permission_apply文件访问权限申请，
         * mentioned_in_doc文档@人，
         * coop_invited邀请加入文件协作，
         * file_commented新增了评论，
         * contact_add_apply新的联系人，
         * group_transfer文件夹/个人团队/企业团队转让
         * 其他
         */
        bussType: string;
        /**
         ...
         * 图标类型
         * 空：前端不显示icon
            file：前端会根据subTitle取后缀显示具体文件类型图标
            folder: 前端会显示成文件夹图标
            sharefolder: 前端会显示成共享文件夹图标
            teamfolder: 前端会显示成团队图标
            enterprise：前端会显示成企业图标
            avatar: 前端会用avatar字段来展示头像
            自定义url：前端直接展示给与的图片url
         */
        icon?: string;
        /**
         * 子标题,可以是文件名（带后缀，方便前端显示各类型图标）、团队名、企业名等等
         */
        subTitle?: string;
        /**
         * 打开参数，具体对照openType说明
         */
        openParam?: Record<string, unknown>;
        /**
         * 打开方式，可选值：blank（新窗口打开）、self（当前窗口），如果为空则blank新窗口打开
         */
        openTarget?: "blank" | "self";
        ...
        action: string;
        /**
         * 消息标题（如：邀请加入协作、申请加入团队等等）
         */
        title: string;
        /**
         * 按钮组
         */
        buttons?: (IMessagebuttonsInfo & {
            /**
             * 按钮展示需要的状态值
             */
            status?: string;
     ...
     * 消息中心按钮点击参数
     */
    export interface IButtonsInterceptInfo {
        /**
         * tab栏 cooperation 办公与协作，contact 新的联系人，schedule日常消息，application应用消息
         */
        category: string;
     ...
     */
    export interface IKdocsPublicSDK extends ModuleBase<{
        kso: KsoModule;
        uikit: UikitModule;
    }> {
        /**
         * 消息中心消息卡片渲染替换(pc/h5) 需要注意卡片高度不能小于100px 1.1.1
         * @param callback messageInfo 消息卡片信息 同时在vue组件中已props的方式传入
         */
        on(action: "MessageCenter.listCard.reset", callback: (messageInfo: IMessageInfo) => BASE.TYPE.VUE): void;
        /**
         * 消息中心气泡消息卡片渲染内容定制(pc/h5) 1.1.1
         * @param callback messageInfo 气泡消息卡片信息
         */
        on(action: "MessageCenter.bubbleMessage.reset", callback: (messageInfo: IMessageInfo) => IMessageInfo | Promise<IMessageInfo>): void;
        /**
         * 消息中心消息卡片底部操作按钮拦截(pc/h5) 1.1.1
         * @param callback
         * ButtonsInterceptInfo 拦截参数
         * return true拦截原本逻辑,false不拦截原本逻辑
         */
        on(action: "MessageCenter.listCard.buttons.intercept", callback: (ButtonsInterceptInfo: IButtonsInterceptInfo) => Promise<boolean>): void;
        /**
         * 分享页顶部功能过滤 1.1.2
         * @param command
         * @param text 文案
         */
        on(action: "FolderSharePage.docslistTopMenu.filter", callback: ISame<{
            command: "saveas" | "download";
            text: string;
        }[]>): any;
        /**
         * 分享页列表更多功能过滤 1.1.2
         ...
         *  @param callback
         */
        on(action: "FolderSharePage.morePanel.filter", callback: ISame<{
            command: "saveas" | "download";
            text: string;
        }[]>): any;
        /**
     ...
     * @interfaceContent
     */
    export interface IKdocsPublicAttach extends InitOption<ISdkLanguageVue> {
        /**
         * 拦截器对象
         */
        interceptors: {
    ...
    export namespace KDOCS_PCWEB {
        type SDK = IKdocsPublicSDK;
        type Attach = IKdocsPublicAttach;
    }
}
