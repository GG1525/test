...
declare module "modules/public/index" {
    ...
    export interface IPublicModule extends ModuleBase<{
        kso: KsoModule;
    }> {
        set(config: IConfig): any;
        on(event: "translate", callback: (word: {
            key: string;
            text: string;
        }) => string): any;
    }
    export class PublicModule extends SdkBase {
    }
}
...
declare module "packages/demo/demo-vue/index" {
    import { KsoModule } from "modules/kso/index";
    import { ModuleBase } from "modules/ModuleBase";
    import { IPublicModule } from "modules/public/index";
    import { UikitModule } from "modules/uikit/index";
    import SdkBase, { LinkEnum } from "libs/SdkBase";
    /**
     * config的配置
     */
    export interface IConfig {
    ...
    export interface IVuePlugSdk extends ModuleBase<{
        public: IPublicModule;
        kso: KsoModule;
        uikit: UikitModule;
    }> {
        /**
        ...
        /**
         * Kso翻译
         * @param callback 需要返回text
         */
        on(action: "ksoui.translate", callback: () => string): any;
        on(action: "uikit.translate", callback: () => string): any;
        on(action: "Customer.component", callback: () => any): any;
        on(action: "Vue.renderObj", callback: () => any): any;
        insertDom(child: Node): () => void;
    }
    export class VuePlugSdk extends SdkBase {
        insertDom(child: Node): () => void;
        links: {
            type: LinkEnum;
            discard: string;
